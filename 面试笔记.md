...
5. timeWaiting（计时等待）：线程调用了带有时间限制的Object.wait()、Object.sleep(T)、Thread.join()等方法后进入该状态，表示线程等待一定时间后自动转入就绪状态
...
在Spring和SpringMvc进行整合时，一般情况下会使用不同配置文件来配置Spring和SpringMvc，因此会产生两个ApplicationContext实例，RootWebApplicationContext、SetvletWebApplicationContext。
