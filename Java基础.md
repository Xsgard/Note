...
1. 初始态【New】，创建了一个线程对象，没有调用start之前的状态
2. 可运行态【Runnable】，线程调用了start()方法后
3. 运行态【Running】，正在执行中的线程
4. 终止态【Terminate】，线程执行结束
5. 阻塞态【Blocking】，又分为三种情况：
   1. 普通阻塞，比如：调用了sleep()方法、join()方法，等待用户输入。
   2. 锁池阻塞，当一个线程拿不到目标资源的互斥锁时，就会进入到目标资源的【对象】的锁池中阻塞
   3. 等待队列，当多个线程进行通信，当前线程调用了目标资源【对象】wait()方法，则，当前线程进入等待队列阻塞，直到其他线程调用了目标资源的notify或者notifyAll方法。

#### 6.1.7 锁资源

> 如果我们呢的代码使用了同步关键字【synchronized】，则就需要使用临界资源对象来达到互斥的目的，如果每个线程都拥有各自的一份临界资源，那么这个临界资源对象就不能在线程之间产生互斥。
>
> **锁 是一种互斥资源，他是对象所拥有的**



#### 6.1.8 synchronized关键字

> 用来做代码同步的，有两种语法：
>
> 1. 直接加在方法的修饰符的位置，表示采用**当前对象**作为临界资源。
> 2. 使用synchronized(对象)方式，这种方式可以自己指定临界资源对象。【*】
...
使用wait()和notify()或notifyAll()来完成

#### 6.2.1 死锁【deadlock】

> 多个线程之间互相申请对方的互斥锁资源时，就会产生死锁。
>
> 写多线程代码时，要尽可能避免。
>
> 如果有多个临界资源时，每个线程在申请临界资源的锁时，尽量保持顺序一致。
...
> java.net包，主要包括如下类型：
>
> 1. ServerSocket  开发基于TCP协议的服务端应用程序
> 2. Socket              开发基于TCP协议的客户端应用程序
> 3. DataGramSocket  开发基于UDP协议的应用程序

#### 7.0.1 TCP

> 是网络通信协议的一种，处于TCP/IP网络模型中的传输层，全称是：Transfer Control Protocol，传输控制协议,他是一种面向连接的网络协议，是安全的。它的建立需要经过三次握手，以保证连接的可靠性。
>
> JDK中封装好的类型**Socket和ServerSocket**

#### 7.0.2 UDP

> 网络协议的一种，处于TCP/IP网络模型中的传输层，与TCP一样。他是非面向连接的，而且不保证数据传输的可靠。

### 7.1.0 开发服务端

1. 创建ServerSocket对象
2. 进行监听，等待客户端的连接，调用accept方法，连接成功就会返回socket
3. 创建一个线程为客户端进行服务，并以Socket为参数



### 7.2.0 开发客户端

1. 创建Socket对象，要指定你要链接的服务端的IP地址和端口
2. 通过Socket来获取输入/输出流或者通道【Channel】
3. 读、写操作
